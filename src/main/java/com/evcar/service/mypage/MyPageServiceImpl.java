...
public class MyPageServiceImpl implements MyPageService {
    ...
    private static final DateTimeFormatter VEHICLE_YEAR_MONTH_FORMATTER =
            DateTimeFormatter.ofPattern("yyyy-MM");
    private static final int MIN_VEHICLE_YEAR = 1900;

    private static final String CONSULT_STATUS_CANCELED = "CANCELED";
    private static final String INQUIRY_STATUS_CANCELED = "CANCELED";
    ...
    public MyPageSummaryResponseDto getMyPageSummary(String userId) {
        ...
        List<MyConsultationResponseDto> consultations =
                getMyConsultations(user.getUserId());

        List<MyInquiryResponseDto> inquiries =
                getMyInquiries(user.getUserId());

        List<MyWishlistResponseDto> wishlist =
                getMyWishlist(user.getUserId());
    ...
    @Override
    @Transactional
    public void updateMyPageInfo(String userId,
                                 MyPageInfoUpdateRequestDto requestDto) {
        ...
        String gender =
                getOrDefault(requestDto.getGender(), user.getGender());

        String phone =
                getOrDefault(requestDto.getPhone(), user.getPhone());

        String address =
                getOrDefault(requestDto.getAddress(), user.getAddress());
        ...
        String email =
                getOrDefault(requestDto.getEmail(), user.getEmail());
        ...
        if (isPasswordChangeRequested(requestDto)) {
            ...
            if (passwordEncoder.matches(
                    requestDto.getNewPassword().trim(),
                    user.getPassword()
            )) {
    ...
    public List<MyConsultationResponseDto> getMyConsultations(String userId) {
        ...
        return consultationRepository
                ...
                .stream()
                .filter(consultation ->
                        !CONSULT_STATUS_CANCELED.equals(
                                consultation.getConsultStatus()
                        )
                )
    ...
    public void cancelMyConsultation(String userId, String consultId) {
        ...
        Consultation consultation =
                consultationRepository.findById(consultId)
                        .orElseThrow(() ->
    ...
    @Override
    public MyConsultationResponseDto getMyConsultationDetail(
            String userId,
            String consultId
    ) {
        ...
        if (CONSULT_STATUS_CANCELED.equals(
                consultation.getConsultStatus()
        )) {
            throw new IllegalArgumentException("취소된 상담입니다.");
        }
    ...
    public List<MyInquiryResponseDto> getMyInquiries(String userId) {
        ...
        return inquiryRepository
                ...
                .stream()
                .filter(inquiry ->
                        !INQUIRY_STATUS_CANCELED.equals(
    ...
    @Override
    public MyInquiryResponseDto getMyInquiryDetail(
            String userId,
            String inquiryId
    ) {
        ...
        if (INQUIRY_STATUS_CANCELED.equals(
                inquiry.getReplyStatus()
        )) {
            throw new IllegalArgumentException("취소된 문의입니다.");
        }
    ...
    @Override
    @Transactional
    public void cancelMyInquiry(String userId, String inquiryId) {
    ...
    public void withdraw(String userId,
        ...
        boolean hasPendingOrInProgressConsultation =
                consultationRepository.existsByUserIdAndConsultStatusIn(
                        user.getUserId(),
        ...
        if (hasPendingOrInProgressConsultation) {
            throw new IllegalArgumentException(
                    "대기중 또는 진행중인 상담이 있어 탈퇴가 불가능합니다. 상담 완료 또는 취소 후 다시 시도해주세요."
            );
        }

        boolean hasWaitingInquiry =
                inquiryRepository.existsByUserIdAndReplyStatus(
                        user.getUserId(),
                        "WAITING"
                );
    ...
    private void validateCurrentPassword(User user,
    ...
    private VehicleInfo resolveVehicleInfo(
            MyPageInfoUpdateRequestDto requestDto
    ) {

        String normalizedHasVehicle =
                normalizeHasVehicle(requestDto.getHasVehicle());

        if ("yes".equals(normalizedHasVehicle)) {

            String vehicleModel =
                    trimToNull(requestDto.getVehicleModel());

            String vehicleYear =
                    trimToNull(requestDto.getVehicleYear());

            Integer drivingDistance =
                    requestDto.getDrivingDistance();

            validateOwnedVehicleInfo(
                    vehicleModel,
                    vehicleYear,
                    drivingDistance
            );
    ...
    private void validatePasswordChangeRequest(
        ...
        if (!hasText(requestDto.getNewPassword())
                || !hasText(requestDto.getNewPasswordConfirm())) {
        ...
        if (requestDto.isInvalidNewPasswordFormat()) {
            throw new IllegalArgumentException(
                    "새 비밀번호는 영문, 숫자만 사용하여 8~20자로 입력해주세요."
            );
        }
    ...
    private boolean isPasswordChangeRequested(
            MyPageInfoUpdateRequestDto requestDto
    ) {

        return hasText(requestDto.getNewPassword())
                || hasText(requestDto.getNewPasswordConfirm());
    }

    private void validateBasicInfo(String name,
                                   LocalDate birthDate,
                                   String gender,
                                   String phone,
                                   String address,
    ...
    private void validatePhone(String phone) {
        ...
        String phoneDigits =
                extractPhoneDigits(normalizedPhone);

        if (!normalizedPhone.matches("^\\+?\\d+$")) {
            throw new IllegalArgumentException(
                    "전화번호는 숫자와 맨 앞의 + 기호만 사용할 수 있습니다."
            );
        }
        ...
        if (phoneDigits.length() < MIN_PHONE_DIGITS
            ...
            throw new IllegalArgumentException(
                    "전화번호는 국가번호 포함 10~15자리 숫자로 입력해주세요."
            );
        }

        if (normalizedPhone.length() > MAX_PHONE_LENGTH) {
            throw new IllegalArgumentException(
                    "전화번호는 최대 16자까지 입력할 수 있습니다."
            );
        }
    }
    ...
    private void validateOwnedVehicleInfo(String vehicleModel,
                                          String vehicleYear,
                                          Integer drivingDistance) {
        ...
        if (!vehicleYear.matches("^\\d{4}-(0[1-9]|1[0-2])$")) {
            throw new IllegalArgumentException(
                    "보유차량 연식은 YYYY-MM 형식이며, 월은 01~12로 입력해주세요."
            );
        }

        YearMonth currentYearMonth =
                YearMonth.now(KOREA_ZONE_ID);

        YearMonth inputYearMonth =
                parseVehicleYearMonth(vehicleYear);
        ...
        if (inputYearMonth.isAfter(currentYearMonth)) {
            throw new IllegalArgumentException(
                    "보유차량 연식은 현재 날짜보다 미래로 입력할 수 없습니다."
            );
        }
    ...
    private String getTrimmedOrDefault(String value,
                                       String defaultValue) {
